...
function LocationPage () {
  ...
  const getCurrentUser = (userInput) => {
    ...
    if(validUser(userInput)){
      ...
      axios.get(`${apiPrefix}/users/${userInput}`)
  ...
  const handleError = (err) => {
    ...
    return(
      ...
      <div className='container mx-auto'>
        ...
        <PageHeading heading='Current location' description = 'we provide opportunity to save the location data on our server.'/>
                        
        <div className='md:pl-xl max-w-xs'>
            <Label for='user_id' className='w-full text-gray-700 text-sm font-bold mb-2'>
              Enter Your User_Id
            </Label>
            <Input id='user_id' name='id' type='number' value={userInput} 
                  innerref={userInputRef} 
                  onChange={(e)=>{handleOnChange(e)}}
                  required
                  />
              
               <Button type='submit' name='submit' onClick={() => getCurrentUser(userInput)}>Submit</Button> 
